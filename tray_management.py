...
tools_state = [-1, -1, -1, -1]
tool_array = [tool(0, 2, 11.1, "A", 0, 3, 19, 16),
                    ...
                    if command == 0:
                        ...
                        if action == 0:
                            ...
                            # if its not primed, check if its loaded
                                ...
                                if api.retract(tool_array[2]) == 1:
                                    tool_array[2].current_state = tool.state.FILAMENT_LOADED
                                    tools_state_queue.put([tool_array[2].tool_number,  tool_array[2].current_state])
                                    tools_prime_state[tool_array[2].tool_number].clear()
                                else:
                                    tool_array[2].current_state = tool.state.FILAMENT_NOT_PRESENT
                                    tools_state_queue.put([tool_array[2].tool_number,  tool_array[2].current_state])
                                    tools_prime_state[tool_array[2].tool_number].clear()
                                # res = command_connection.perform_simple_code("T0")
                                if api.prime_extruder(tool_array[0], tools_prime_state) == 1:
                                    tool_array[0].current_state = tool.state.FILAMENT_PRIMED
                                    tools_state_queue.put([tool_array[0].tool_number,  tool_array[0].current_state])
                                    tools_prime_state[tool_array[0].tool_number].set()
                            else:
                            ...
                            # if its not primed, check if its loaded
                                ...
                                # retract other tool just in case
                                if api.retract(tool_array[0]) == 1:
                                    tool_array[0].current_state = tool.state.FILAMENT_LOADED
                                    tools_state_queue.put([tool_array[0].tool_number,  tool_array[0].current_state])
                                    tools_prime_state[tool_array[0].tool_number].clear()
                                else:
                                    tool_array[0].current_state = tool.state.FILAMENT_NOT_PRESENT
                                    tools_state_queue.put([tool_array[0].tool_number,  tool_array[0].current_state])
                                    tools_prime_state[tool_array[0].tool_number].clear()
                                # res = command_connection.perform_simple_code("T0")
                                if api.prime_extruder(tool_array[2], tools_prime_state) == 1:
                                    tool_array[2].current_state = tool.state.FILAMENT_PRIMED
                                    tools_state_queue.put([tool_array[2].tool_number,  tool_array[2].current_state])
                                    tools_prime_state[tool_array[2].tool_number].set()
                            else:
                    ...
                    # Change filament in extruder 0
                        ...
                        if action == 0:
                            ...
                            # if its not primed, check if its loaded
                                ...
                                # retract other tool just in case
                                if api.retract(tool_array[3]) == 1:
                                    tool_array[3].current_state = tool.state.FILAMENT_LOADED
                                    tools_state_queue.put([tool_array[3].tool_number,  tool_array[3].current_state])
                                    tools_prime_state[tool_array[3].tool_number].clear()
                                else:
                                    tool_array[3].current_state = tool.state.FILAMENT_NOT_PRESENT
                                    tools_state_queue.put([tool_array[3].tool_number,  tool_array[3].current_state])
                                    tools_prime_state[tool_array[3].tool_number].clear()
                                # res = command_connection.perform_simple_code("T0")
                                if api.prime_extruder(tool_array[1], tools_prime_state) == 1:
                                    tool_array[1].current_state = tool.state.FILAMENT_PRIMED
                                    tools_state_queue.put([tool_array[1].tool_number,  tool_array[1].current_state])
                                    tools_prime_state[tool_array[1].tool_number].set()
                            else:
                                #pointless to continue
                                print("not even loaded")
                        else:
                            ...
                            # if its not primed, check if its loaded
                                ...
                                # retract other tool just in case
                                if api.retract(tool_array[1]) == 1:
                                    tool_array[1].current_state = tool.state.FILAMENT_LOADED
                                    tools_state_queue.put([tool_array[1].tool_number,  tool_array[1].current_state])
                                    tools_prime_state[tool_array[1].tool_number].clear()
                                else:
                                    tool_array[1].current_state = tool.state.FILAMENT_NOT_PRESENT
                                    tools_state_queue.put([tool_array[1].tool_number,  tool_array[1].current_state])
                                    tools_prime_state[tool_array[1].tool_number].clear()
                                # res = command_connection.perform_simple_code("T0")
                                if api.prime_extruder(tool_array[3], tools_prime_state) == 1:
                                    tool_array[3].current_state = tool.state.FILAMENT_PRIMED
                                    tools_state_queue.put([tool_array[3].tool_number,  tool_array[3].current_state])
                                    tools_prime_state[tool_array[3].tool_number].set()
                            else:
                                #pointless to continue
                                print("not even loaded")
                    # Change filament in extruder 0
